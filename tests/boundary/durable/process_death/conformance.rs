...
/// Compaction is not a configuration publication boundary: the request issued after a
...
fn compaction_never_refreshes_resource_derived_authority() {
    ...
    assert_eq!(
        ...
        "compaction is not a configuration publication boundary"
    );
}
...
fn invalid_current_resources_fail_runtime_creation() {
    ...
    std::fs::write(lab.root().join("rustx.toml"), "{ not json").expect("corrupt the config");
