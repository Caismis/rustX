...
fn projection_ordering_is_deterministic() {
    ...
    let first = engine
        .build_projection(&history, Some(&checkpoint), &[], None)
        .expect("projection");
    let second = engine
        .build_projection(&history, Some(&checkpoint), &[], None)
...
fn provider_reported_usage_applies_only_to_the_exact_projection() {
    ...
    let estimated = engine
        .build_projection(&grown, None, &[], Some(&observed))
...
fn threshold_equality_compacts() {
    ...
    let at = engine
        .build_projection(&vec![user("u1", ""); 5], None, &[], None)
        .expect("projection");
    ...
    let below = engine
        .build_projection(&vec![user("u1", ""); 4], None, &[], None)
        .expect("projection");
    ...
    let above = engine
        .build_projection(&vec![user("u1", ""); 6], None, &[], None)
        .expect("projection");
...
fn repeated_compaction_after_an_inside_agent_checkpoint() {
    ...
    let projection2 = engine
        .build_projection(&grown, Some(&checkpoint1), &[], None)
        .expect("second projection");
    let second = engine
        .plan_compaction(&grown, Some(&checkpoint1), &projection2, &[], 0, None)
        .expect("second plan");
    assert_eq!(
        second.boundary,
        ContextBoundary::AfterMessage {
...
fn incremental_second_checkpoint_receives_only_new_material() {
    ...
    let projection2 = engine
        .build_projection(&history, Some(&checkpoint1), &[], None)
        .expect("second projection");
    let second = engine
