...
const INITIALIZE: &str = r#"{"jsonrpc":"2.0","id":1,"method":"initialize","params":{"protocol_version":19,"client":{"name":"boundary","version":"1"},"presentation":{"images":false,"questionnaires":false,"reviews":false}}}"#;
...
async fn app_server_websocket_authentication_framing_and_protocol_errors() {
    bounded(async {
        ...
        for offer in [
            None,
            Some("rustx.app-server.v19"),
            Some(old_offer.as_str()),
            Some("rustx.app-server.v19, rustx-token.wrong"),
        ] {
