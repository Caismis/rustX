...
fn adapter(server: &crate::common::FixtureServer) -> OpenAiResponsesAdapter {
...
async fn openrouter_responses_dialect_is_normalized() {
    let reasoning_server = crate::common::FixtureServer::start(|_attempt, _head| {
    ...
    let error_server = crate::common::FixtureServer::start(|_attempt, _head| {
...
async fn model_selectable_execution_mode_schema_reaches_the_provider_verbatim() {
    ...
    request.tools = vec![crate::common::model_selectable_tool("bash", "tool-bash")];
    let events = collect_events(
        &adapter(&server),
        with_storage(request, ResponsesStorageMode::Stored),
    )
    ...
    crate::common::assert_compiled_execution_mode_schema(&tool["parameters"], "command");
}
