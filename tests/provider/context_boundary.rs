...
async fn empty_effective_prompt_emits_no_system_authority_in_any_protocol() {
    ...
    crate::common::collect_events(&chat, chat_request).await;
    ...
    crate::common::collect_events(&responses, responses_request).await;
    ...
    let anthropic_server = crate::common::FixtureServer::start(|_attempt, _head| {
        crate::common::sse_fixture("anthropic", "text.sse")
    })
    .await;
    ...
    crate::common::collect_events(&anthropic, anthropic_request).await;
...
fn request_snapshot_reconstructs_exactly_after_live_state_changes() {
    ...
    let snapshot = RequestSnapshot::new(
        ...
        crate::common::invocation(ModelProtocol::OpenAiChatCompletions, "frozen-model"),
        128_000,
        None,
        false,
        Vec::new(),
