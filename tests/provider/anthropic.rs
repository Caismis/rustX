...
fn adapter(server: &crate::common::FixtureServer) -> AnthropicMessagesAdapter {
...
async fn refusal_without_explanation_does_not_fabricate_text() {
    let server = crate::common::FixtureServer::start(|_attempt, _head| {
        ...
        reply.chunks.clear();
        reply.chunks.push(crate::common::FixtureChunk {
            delay_ms: 0,
            bytes: [
...
async fn model_selectable_execution_mode_schema_reaches_the_provider_verbatim() {
    ...
    assert_compiled_execution_mode_schema(&tool["input_schema"], "command");
}
