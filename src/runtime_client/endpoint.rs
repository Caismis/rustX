...
//! entry point of Runtime Client Protocol v4.
//!
//! [`RuntimeClientEndpoint`] is the boundary every transport wraps. It
//! accepts *every* v4 request — including `initialize` — and returns the
...
impl RuntimeClientEndpoint {
    ...
    /// Handles one Runtime Client Protocol v4 request and returns its
    /// correlated response.
    ///
