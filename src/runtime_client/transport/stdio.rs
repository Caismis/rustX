...
//! does not deserialize to the exact Runtime Client request type — malformed JSON,
...
//! protocol record is written, and no semantic request is applied. The
//! Runtime Client protocol has no uncorrelated error envelope, and this
//! transport does not invent one.
//!
//! # Session shape
//!
//! ```text
...
///
/// The same record limit applies in both directions. It bounds the JSON payload
    ...
    fn fmt(&self, f: &mut core::fmt::Formatter<'_>) -> core::fmt::Result {
        match self {
            ...
            Self::MalformedRecord { message } => {
                write!(
                    f,
                    "an inbound record is not a valid Runtime Client request: {message}"
                )
            }
        }
    }
...
pub enum StdioTransportError {
    ...
    /// the transport was stalled. The Runtime Client protocol has no uncorrelated
...
/// the public API exposes only the frozen record limit.
async fn serve<R, W>(
    endpoint: RuntimeClientEndpoint,
    reader: R,
    writer: W,
...
/// Decodes one complete in-bound-size record to the exact Runtime Client request type.
...
mod tests {
    ...
    fn invalid_records_never_decode() {
        for record in [
            ...
            &br#"{"method":"initialize","id":"one","protocol_version":6}"#[..],
            &br"[]"[..],
        ] {
            assert!(
                matches!(
