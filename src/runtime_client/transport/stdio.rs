...
//! does not deserialize to the exact v1 request type — malformed JSON,
...
//! v1 has no uncorrelated error envelope, and this transport does not
//! invent one.
//!
//! # Session shape
//!
...
///
/// The same v1 limit applies in both directions. It bounds the JSON payload
...
pub enum StdioFramingError {
    ...
    /// Runtime Client Protocol v1 request type.
    MalformedRecord {
    ...
    fn fmt(&self, f: &mut core::fmt::Formatter<'_>) -> core::fmt::Result {
        match self {
            ...
            Self::MalformedRecord { message } => {
                write!(f, "an inbound record is not a valid v1 request: {message}")
            }
        }
    }
}
...
pub enum StdioTransportError {
    ...
    /// the transport was stalled. Protocol v1 has no uncorrelated
...
/// the public API exposes only the frozen v1 limit.
async fn serve<R, W>(
    endpoint: RuntimeClientEndpoint,
    reader: R,
    writer: W,
...
/// Decodes one complete in-bound-size record to the exact v1 request type.
