...
async fn cfg236_gated_role_reload_cancels_or_publishes_one_complete_generation() {
    ...
    let reload = tokio::spawn(async move { runtime.reload_configuration().await });
...
async fn project_symlink_escape_is_rechecked_before_composition_and_reload() {
    let f = Fixture::new();
    let outside = f.root.path().join("outside.toml");
    std::fs::write(&outside, "SECRET_EXTERNAL_INVALID: [").unwrap();
    let source = f.role(false, "x", json!({"description":"x"}), "captured body");
    f.project(json!({"agent":{"agents":["x"]}}));
    let launch = f.resolve();
    // Already captured bytes remain immutable even before the first composition.
    std::fs::write(&source, "malformed edit").unwrap();
    let product = LocalSessionClient::compose(&launch, &LocalRuntimeDependencies::default())
        .await
        .unwrap();
    ...
    assert!(resolve(&f.request, &f.host).is_err());
    let error = product.runtime().reload_configuration().await.unwrap_err();
    assert!(!error.to_string().contains("SECRET_EXTERNAL_INVALID"));
    assert!(std::sync::Arc::ptr_eq(
        &before,
        &product.runtime().runtime_resources()
    ));
    // Unselected malformed resources warn; they never restore a lower definition.
    f.project(json!({"agent":{"agents":[]}}));
    product.runtime().reload_configuration().await.unwrap();
    let after = product.runtime().runtime_resources();
    assert_ne!(before.revision(), after.revision());
    assert!(!after.inspection().resource_diagnostics.is_empty());
    std::fs::remove_file(&source).unwrap();
    product.runtime().reload_configuration().await.unwrap();
    product.runtime().shutdown().await.unwrap();
}
...
async fn workspace_workflow_symlink_rejects_reload_without_reading_external_yaml() {
    ...
    f.project(json!({"agent":{"workflows":["escape"]}}));
    let error = product
        .runtime()
        ...
        .to_string();
    assert!(
        error.contains("outside workspace boundary"),
...
async fn cfg271_all_catalogs_publish_together_and_failed_candidates_publish_nothing() {
    ...
    f.project(json!({"agent":{"agents":["added"],"workflows":["example"]}}));
    assert!(product.runtime().reload_configuration().await.is_err());
    assert!(std::sync::Arc::ptr_eq(
        &before,
        &product.runtime().runtime_resources()
    ));
    ...
    let runtime = product.runtime().clone();
    let mut reload = tokio::spawn(async move { runtime.reload_configuration().await });
    ...
    // A selected malformed Agent or Workflow rejects the whole candidate.
    ...
    // An unused malformed Skill warns while unrelated selected resources publish.
    ...
    f.project(json!({"agent":{"agents":[],"workflows":[]}}));
...
async fn cfg274_skill_recovery_and_loss_publish_workflow_and_root_exposure_atomically() {
    ...
    f.project(json!({"agent":{"workflows":[],"tools":{"builtin":[]}}}));
    let product = LocalSessionClient::compose(&f.resolve(), &LocalRuntimeDependencies::default())
        .await
        .unwrap();
...
async fn cfg275_child_inspection_and_execution_share_frozen_r1_after_parent_reload() {
    ...
    use crate::runtime::subagent::resolver::SubagentResolution;
    use crate::runtime::subagent::{SubagentName, SubagentResolver};
    let f = Fixture::new();
    f.project(json!({"agent":{"tools":{"builtin":["read"]},"agents":["reviewer"],"skills":["root-visible"]}}));
    ...
    f.role(false, "reviewer", json!({"description":"Review", "tools":{"builtin":["read","grep"]}, "skills":["rust-review"], "plugins":{"todo":{"enabled":false},"agent_status":{"enabled":true}}}), "SECRET_R1_INSTRUCTIONS");
    let launch = f.resolve();
    ...
    f.role(false, "reviewer", json!({"description":"R2", "tools":{"builtin":["bash"]},"skills":["root-visible"],"plugins":{"todo":{"enabled":true},"agent_status":{"enabled":false}}}), "R2 instructions");
    parent.runtime().reload_configuration().await.unwrap();
