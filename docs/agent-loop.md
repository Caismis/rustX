...
`completion`. Once a deadline/cancellation intent wins, the lifecycle awaits
ONLY the executor's settlement authority — the normal settlement mechanism,
awaited without a timeout of its own — and never polls `completion` again.
Cancellation *intent* (the deadline or attempt winner) belongs to the
lifecycle; physical cancellation is executor-owned: the intent is delivered
through the execution's `ExecutionCancellation`, and the executor's
settlement plane resolves only after it observed the request and ran its
physical cancellation path to its end — including all rustX-owned local
cleanup (kill, wait, reap, join) — returning typed `ToolSettlement`
evidence: `Confirmed(result)` for proven physical terminality, or
`Unconfirmed { detail }` when local rustX execution ownership reached its
terminal cleanup boundary (no rustX-owned task or process remains) but
terminality past the external-effect frontier cannot be proven — for
example a dispatched remote MCP call. The settled evidence selects the
canonical status under the Issue
#202 outcome-certainty contract:
...
- **settlement control-plane guard expiry** → `OutcomeUnknown`:
  ...
  executor whose settlement plane never returns after observing the
  cancellation request. Its expiry is a settlement control-plane failure (an
  executor settlement-contract violation), never settlement evidence — it
  never implies the physical operation stopped — so the lifecycle never
  reports `TimedOut` it cannot prove.

Executor-returned `Unconfirmed` and guard expiry stay type-distinct
lifecycle facts, never collapsed: the executor's `ToolSettlement` is
settlement evidence returned by the executor's authority, while
...
`ToolExecutionHandle::settled_by_operation` the operation's ownership moved
to the settlement plane, and in any conforming executor the completion
future is by then an empty poller shell. A conforming executor never spawns
unmanaged rustX-owned local tasks or processes outside its handle futures,
so on the guard path dropping the handle consumes all remaining local
execution ownership; external systems may remain unknown, but no
rustX-owned local execution outlives it. The committed terminal result is
absorbing: the closed call slot guarantees that no late physical completion,
no residual executor-owned physical cleanup, and no repeated intent can
publish a second result, a post-terminal progress/cancellation/settlement
fact, or rewrite canonical history — the call's slot never reopens.
...
Per started call the Event Journal records, in order:
`ToolExecutionStarted`, the retained `ToolExecutionProgress` facts,
`ToolExecutionDeadlineFired { kind }` when a deadline fired,
`ToolExecutionCancellationRequested { cause }` when a non-physical winner
drove the settlement phase, then exactly one settlement fact —
`ToolExecutionSettlementObserved { certainty }` when the executor's
settlement authority returned typed evidence, or
...
The journal is observational evidence, and the canonical ToolResult remains
...
Drain waits for the attempt's *canonical* settlement of each started
foreground execution, and that settlement now awaits the executor's
settlement authority (§7.1.2): a `Confirmed` report, an `Unconfirmed`
report, or the settlement control-plane guard. An executor-returned
`Unconfirmed` carries the local-ownership invariant — all rustX-owned local
execution ownership of the call already settled, only the external effect
beyond rustX's ownership domain remains uncertain — so drain may complete
past it: external uncertainty never blocks local quiescence, while residual
rustX-owned local cleanup always does (the settlement plane stays pending
until the cleanup completes). On the guard path the lifecycle drops the
handle, which for a conforming executor consumes all remaining local
execution ownership, before drain can reach quiescence. The closed call
slot seals the settled call from canonical history: no late state can ever
publish a canonical fact after drain returns.
