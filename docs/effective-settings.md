...
| Test | Boundary/evidence |
| --- | --- |
| `ext259_ordinary_tool_selection_neither_adds_nor_removes_the_extension_tool` | Every ordinary activation shape, including `--no-direct-tools` and `--no-builtin-tools`, against both compositions; the extension Tool is present or absent purely by composition, and never appears in the ordinary available catalog |
