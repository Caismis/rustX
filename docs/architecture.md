...
App Server v18 prepares a scoped streaming-download capability; Web consumes it
...
See [App Server protocol v18](app-server-protocol.md) for the method vocabulary,
...
which after #290 has no external client: `rustx-tui` speaks App Server v18, and
...
App Server v18 projects authored sources, effective configuration and composable
...
Both are thin App Server v18 clients. TUI `/settings` authors User/Workspace
...
The exact pending controls described in [App Server protocol v18](app-server-protocol.md#exact-pending-inbound-controls-web-06)
...
the existing root interaction owner. See [the protocol](app-server-protocol.md#read-only-native-subagent-conversations-v18).
