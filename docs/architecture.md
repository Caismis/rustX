...
bounded repetition detector. Its whole state is a fixed 2 KiB ring window, one
64-byte checkpoint block, and three counters; nothing grows with the length of
the generation, and per-byte work is bounded, so total detector work is linear
in the generated bytes and never quadratic. Detection is exact suffix
periodicity: at each cumulative-byte checkpoint the window suffix is tested
against every candidate period, with constant-time sample probes rejecting
almost all of them before the linear verification runs.
...
**Output and reasoning budgets.** These are semantically distinct: a model may
legitimately answer at length, and it may legitimately think before answering,
but "may think without bound" does not follow from "may answer at length".
...
depend on a self-hosted backend honoring its own request limit. It is derived
deterministically from the already-resolved budget and its guarantee is stated
exactly:

> One physical generation cannot stream more than `max_generated_bytes` of
> normalized generated data, nor more than `max_reasoning_bytes` of normalized
> reasoning text, before the runtime terminates it. Both numbers are UTF-8
> **byte** counts of the adapter's normalized output, **not model tokens**,
> and neither replaces the provider-enforced `max_output_tokens`.

The derivation is `32 bytes per resolved output token`, floored at 256 KiB and
with reasoning granted three quarters of the result. The multiplier is
deliberately generous — real output runs nearer four bytes per token — because
this bound exists to stop a runaway stream, not to shape an answer: a bound
that could plausibly reject a legitimate long generation would be the worse
error. The repetition detector, not this floor, is what catches the failure
mode Issue #203 observed, at roughly a kilobyte of evidence.
...
**Typed facts, bounded by construction.** All three outcomes normalize into
the existing model-error vocabulary rather than new top-level runtime events:

| Condition | `ModelErrorKind` | `ModelError.generation` |
| --- | --- | --- |
| repetition loop | `GenerationDegenerated` | `Degenerated { channel, period_bytes, repetitions, span_bytes }` |
| provider token limit | `GenerationBudgetExceeded` | `ProviderLengthLimit` |
| runtime byte safeguard | `GenerationBudgetExceeded` | `RuntimeBudgetExceeded { budget, limit_bytes, observed_bytes }` |
| expired deadline | `Timeout` | `Timeout { phase }` |

Every payload is an enumeration or an integer, so provider output can never
author unbounded durable diagnostic data through this type: the bound is
...
Journal; no new `RuntimeEvent` variant was needed. The deadline phase is now a
typed `ModelTimeoutPhase` rather than a substring of a message, so no consumer
above the model layer reads prose to learn which contract expired.
...
from native, MCP (managed Python packages included), and future source
registrations, applies hard
eligibility, then applies startup activation. Its
candidate preparation builds one available Tool catalog from native, MCP,
and future source registrations, applies hard eligibility, then
...
`prepare_selected_candidate` instead, which connects the frozen
`python:<folder>` server bindings exactly like any selected MCP server —
there is no Python-specific child materialization channel. A failed initialization leaves the slot empty so the
...
single build-coalescing domain), never reconstructed per
preparation.
...
- **Skill.** The immutable `SkillId` + `SkillVersionId` binding of each
  ...
  being silently omitted. #145 makes the version binding authoritative in the
  child as well by materializing the exact frozen bytes and re-proving the
  digest over them (see *Skill materialization* below).
...
disposable live observation projections. The observation plane projects what
a child is doing *right now* without creating a second authority:
...
SubagentRegistry::apply_activity
        ...
        |               records and non-advancing revisions (child-owned,
        |               strictly monotonic)
        v
...
on whether a child is alive, settling, or settled. `SubagentActivity`
(`awaiting_activity`, `model`, `retrying_model`, `tool`, `compacting`,
`waiting`) is an orthogonal live projection derived only from objective
child runtime transitions; it is never a lifecycle state, and a child can be
lifecycle-`Cancelling` with any activity still projected.

The reliability split is deliberate. Lifecycle, cancellation, and terminal
publication remain reliable and exactly-once; only superseded activity
observations are coalescible — a watch overwrite or a replay-ring eviction
drops them, and consumers converge on the newest projection through
`resync_required` and snapshot repair. Observation delivery or drop never
determines terminality: a zero-observer or stalled-observer execution
behaves identically to an observed one. At settlement the registry resets
the activity to the terminal-neutral `awaiting_activity` with a revision
...
child actually started with, it carries only the effective model identity
and the reasoning selection — never credentials, endpoints, or provider
internals. Observing a child never enlarges parent model context: the
model-facing `execution` projection drops the observation-plane fields, and
the successful terminal answer flows only through the canonical durable
terminal inbound publication.

Nothing in the observation plane is durable or recovered. After a restart a
recovered child projects the initial absent observation (revision 0,
...
`UserSource::Agent(parent)`. The registry's committed `CancellationReason`
travels through the driver's typed command and a child-side `Cancel(reason)`
into the runtime-owned one-shot cancellation intent
...
#### External subagent capabilities and nested process containment (Issue #145)

##### Semantic authority versus physical realization

The architecture is one sentence:

> The parent freezes semantic authority. The child physically realizes
> exactly that frozen authority, using runtimes it owns itself.
...
ResolvedSubagentSpec                      (parent: decides)
  ...
  ├─ frozen project instructions
  ├─ FrozenModelSpec
  └─ ResolvedSubagentMaterialization      (only the required sources)
        |
        v  one bounded IPC plane
child process                             (child: realizes)
  ...
  └─ registers exactly the frozen Builtins
        |
        v
ONE ordinary child ToolRegistry -> ConversationRuntime / Agent Loop
```

The core invariant is:

> A subagent executes exactly the capability/resource identities frozen for
> its spawn, or it does not start.

There is still exactly one Tool Plane in the child. #145 adds no MCP subagent
registry, no Python subagent registry, no Skill-specific Tool Plane, and no
second capability model, and it removed the temporary #144 refusal of
externally sourced capabilities rather than keeping both modes.

##### Selected-only child materialization

The child does **not** run the parent's discovery pipeline with a filter
bolted on. `CapabilityCoordinator::prepare_selected_candidate` is a separate
physical path whose inputs are frozen identities
(`SelectedCapabilityPlan`), and "selected only" is structural rather than a
rule someone has to remember:

```text
no Skill discovery              the frozen packages are materialized, never found
no workspace Python discovery   Python tools arrive as frozen `python:<folder>`
                                MCP bindings; the child never opens a Python store
no "every configured server"    only the frozen bindings exist in the child at all
no activation policy pass       the frozen set IS the active set
```

`ResolvedSubagentMaterialization` carries **only** the sources the selection
needs: an agent that selects one GitHub MCP tool receives one binding, so
there is no second server for the child to connect even in principle. A
required source that cannot be realized is a **preparation failure**, never
an availability degradation: the child starts with exactly the authority it
was authorized with or it does not start, and the failure happens before it
answers `Ready` and therefore long before any durable ownership commit.

##### MCP cross-process semantic identity

The process-local MCP invalidation **epoch** stabilizes one process's catalog
read and means nothing in another OS process, so it cannot certify a child
generation. `MCP_TOOL_IDENTITY_V1` is the rustX-owned, versioned, canonical
identity that can:
...
numbers use their exact integer form or Rust's shortest round-trip decimal
form. The digest therefore does not depend on `serde_json`'s map
implementation, its feature flags, or insertion order.

The parent freezes the expected digest. The child then, in order: starts and
connects **only** the required server, performs its own `tools/list`, derives
the canonical identity of what the server actually publishes, compares it to
the frozen expectation, fails preparation before any ownership commit if the
definition is missing or changed, and only then constructs a child-local
executor from a child-owned runtime binding. No parent executor, lease,
transport handle, or process-local epoch ever crosses the process boundary.

##### Python: one fingerprint-keyed preparation store

The managed Python store (`PythonToolStore`) is rooted at
`<environment store>/python-tools`, inside the coordinator's own
environment store:

```text
<environment store>/python-tools/
├── packages/<fingerprint>/      # one immutable prepared state per fingerprint
│   ├── source/                  # frozen package source copy
│   ├── pyproject.toml           # rustX-generated project stub
│   ├── uv.lock                  # rustX-generated lock
│   ├── venv/                    # UV_PROJECT_ENVIRONMENT target
│   └── manifest.json            # rustX-owned identity/digest manifest
└── uv-cache/                    # shared uv cache (scratch state)
```

`packages/<fingerprint>/` states are immutable and content-keyed: publication
is a sibling staging directory plus one atomic `rename` with an explicit
concurrent-winner validation path, and every read is fail-closed — the
manifest, the identity inputs, the frozen source digest, and the venv
interpreter all re-verify or the state is an explicit preparation failure,
never a silent reuse and never a repair. `uv-cache/` is shared scratch
because **uv** supports concurrent readers and writers against one cache and
owns its own cache locking — which is a statement about uv, not about rustX:
it does *not* imply that rustX's publication state is cross-process safe. The
in-flight build coalescing is one `Arc<Mutex<..>>` inside one process, which
is not cross-process synchronization, and rustX deliberately does not add a
cross-process lock to pretend otherwise.
...
model-visible catalog location onto its own copy. Automatic Skill discovery
is never restarted, and progressive disclosure is untouched: only bytes on
...
External composition may start an MCP process, negotiate a protocol, and list
a catalog — a selected managed Python tool is one such MCP connect, against
its synthesized server's prepared venv interpreter. `ChildPreparation`
therefore races the composition against two
settlement authorities that are **futures, not polled mutable state**:
...
> Cancellation, parent loss, or preparation failure before subagent ownership
...
(The parent runtime's own candidate preparation applies the same discipline
to its Python runtime-identity probes and uv lock/sync builds, whose #153
owner task captures the establishing preparation's authority, so waiters
never cancel shared builds.) The guard observes the authority, then
...
Once a child may run Bash, MCP stdio (a synthesized `python:<folder>` server
included), or Skill environment work,
the process tree gains a level rustX never had before:
...
Killing the child's process group reaches the child and the outer supervisor
but **not** the inner group — `setsid()` put it in a different session by
construction, which is exactly what makes the unit's own `TERM`/`KILL`
discipline correct. `crate::runtime::nested_containment` inserts the
top-level parent into the gate every supervised unit already has, once,
generically, for every unit created through the shared supervised-unit path
(native Bash, MCP stdio — a synthesized Python tool server's venv interpreter
included —, Skill environment
subprocesses):
...
> A nested supervised unit may not cross its local start/ownership gate until
> the top-level parent has acknowledged retention of that exact containment
> anchor.

The acknowledgement is part of the ownership protocol, not telemetry: it
splits child death into two provably different worlds. Before the ACK the
inner never received `START`, so the semantic command was never spawned;
after the ACK the parent holds the exact `pgid` and owns catastrophic
containment for it. There is one containment abstraction — no
`McpSubagentAnchor` or `BashSubagentAnchor`.

In the top-level process no authority is installed, so the gate resolves
immediately and the existing single-level behaviour is unchanged. The
`AnchorGate` is polled as its own arm of each owner's settlement loop, so
...
Parent-side containment reuses the proven primitive shape: retain the adopted
anchor with `WNOWAIT` (proving the numeric identity is still allocated and
owned here, so signalling a cached `pgid` is legal), issue one anchored
`SIGKILL`, and prove the group terminal with a group-scoped `ECHILD`
(`contain_adopted_group`). It is the sibling of `emergency_contain_group`:
that one first waits for the inner supervisor because the inner is still the
unit's own reaping owner, while here the whole owning rustX process is gone
and nothing will ever drive the unit to its terminal state.

Every step requires the anchor to have been **adopted** when the intermediate
child died. On Linux that is `PR_SET_CHILD_SUBREAPER`, and it must exist
*before* the child is spawned — a subreaper installed afterwards does not
retroactively adopt. `spawn_staged` therefore consults
`ensure_child_subreaper` first and fails with
`SpawnError::ContainmentPrerequisite` rather than claiming containment it
cannot perform.

On macOS and other platforms without an orphan-adoption primitive, an
orphaned anchor reparents to `init`, the anchor wait answers `ECHILD`, and
containment reports `AnchorUnavailable`. That is published as explicitly
**unproven** terminality — the existing platform contract — and is never
fabricated into Linux parity.

##### Staged versus committed anchor ownership

Nested units can be created during child preparation, before any durable
ownership commit, so the staged owner must own them:

```text
StagedChild
  ├─ the direct child process
  └─ retained nested ProcessUnit anchors
        |  exactly-once transfer at the ownership commit
        v
child driver task (registry-owned)
```

The set is **moved**, never copied, so there is one owner at every instant.
Pre-commit, `StagedChild::rollback` kills and reaps the direct child and then
settles every retained anchor; rollback returns
...
The driver task therefore does not return — and the registry's counted
lifecycle admission is not released, so runtime drain cannot declare
quiescence — until every retained unit is contained or explicitly reported
unprovable. An unproven nested settlement never changes the child's semantic
terminal, but it is never silently dropped either: it is appended to the
terminal's own bounded diagnostic. An anchor is never removed merely because
...
partial frames, lost EOF observation, and acknowledgements delivered to
whichever task happened to be reading. There is instead exactly one
dispatcher:
...
- **Retry is child-local (Issue #134).** A transient provider failure inside
  ...
  state), or inbound. Since Issue #178 the retry is visible only as live
  activity — the child projects `retrying_model` and then
  `model { retry }` through the observation plane — a disposable read model
  that is never a lifecycle state, a journal fact, or an inbound and never
  moves retry ownership.
- **The frozen `ModelTimeoutPolicy` is inherited (Issue #135).** The
  ...
  `SubagentChildSpec` (subagent control protocol version 8); the child
...
- **A named child is an ordinary conversation runtime.** Named-definition
  resolution freezes the full `ResolvedSubagentSpec` before launch, and
  child-owned preparation materializes the exact Builtin, MCP (managed Python
  packages included), and
  Skill identities under the #145 containment boundary. The resulting child
  runs retry, timeout, tool-cancellation, terminal publication, and
  unresolved-output carryover in its own ordinary `ConversationRuntime`/
  Agent Loop. Retry state, publication audit, and pending Carryover remain
  conversation-local; a failed one-shot child exposes no partial model output
  to its parent.
- **Cancellation is the ordinary explicit path.** The parent registry
  commits the first `CancellationReason` exactly once and transports it as
  `DriverCommand::Cancel { reason }` through the typed IPC
  `ParentFrame::Cancel { reason }` into the child's ordinary
  `cancel_current_or_next_attempt(reason)` path. The process driver
  transports but never chooses the semantic reason; parent and child remain
  separate cancellation authorities, with no retry-specific control message
  and no shared cancellation token. A cancellation or drain that wins while
  the child sleeps in retry backoff loses the backoff race deterministically:
  no next `ModelRequestStarted` commits and the child settles
  `AttemptCancelled` with that same reason. Runtime drain therefore retains
  `RuntimeShutdown` provenance rather than being relabeled `UserRequested`.
...
- **Failed children expose no partial output.** A failed/cancelled
  ...
  wire).
- **Process interruption is not provider retry.** A semantic child model
  failure is `Failed`; unexpected child process/IPC/runtime loss without a
  valid semantic terminal is `Interrupted`, not model `Failed`. It is never
  provider retry and never triggers automatic relaunch of the delegated task.
  A process killed by driver escalation after an already-committed
  cancellation remains `Cancelled` with that committed reason. If process,
...
unknown and MCP tools (managed Python packages included) are always fully
usable.
