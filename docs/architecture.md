...
App Server v22 prepares a scoped streaming-download capability; Web consumes it
...
See [App Server protocol v22](app-server-protocol.md) for the method vocabulary,
...
which after #290 has no external client: `rustx-tui` speaks App Server v22, and
...
App Server v22 projects authored sources, effective configuration and composable
...
Both are thin App Server v22 clients. TUI `/settings` authors User/Workspace
...
native coordinator; the focused Session header owns explicit adoption. Preparation
failure and process restart can appear together. Conflicts preserve target-scoped
drafts; uncertain outcomes trigger reads, never blind replay.
...
The exact pending controls described in [App Server protocol v22](app-server-protocol.md#exact-pending-inbound-controls-web-06)
...
the existing root interaction owner. See [the protocol](app-server-protocol.md#read-only-native-subagent-conversations-v22).
