...
Instructions composition includes Root instructions, `agents_md`, context policy,
project-context and Root project inputs (including Workspace paths retained for
physical-authority validation), effective fields, their provenance and
Instructions component identity. Runtime context composition also moves Provider
...
| T03 | `t03_t06_t09_t15_t16_healthy_registration_preserves_complete_policy_authority` proves Workspace policy value/effective/provenance agreement, separate policy/capacity identities, no deferred/application/candidate, zero load preparation and stable resource pointers. The mixed-failure regression verifies User I1 → Workspace I2 provenance in source and runtime, C1/I2 component identities, the retained diagnostic source manifest and S2’s unresolved C2 relationship. Replacing the composed I2 project file with an escaping symlink fails physical-authority validation. Strengthened `t03_t05_failed_capabilities_preserve_leases_while_instructions_are_adopted` proves C1+I2 availability for a brand-new Session and C2+I2 after retry. Headless `t03_t05_failed_capabilities_preserve_leases_while_instructions_are_adopted`: failure injected after construction retires the uncommitted capability candidate; instruction adoption retains the exact registry and Skill lease identities and failed capability status. Also `t03_mixed_application_and_true_process_binding_restart`; coordinator `t03_units_have_simultaneous_independent_outcomes`; Web/TUI mixed-state tests below. |
