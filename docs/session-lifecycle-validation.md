...
mandatory protocol is **v16**. `rustx.app-server.v16` is the only WebSocket
subprotocol. Rust initialization, TUI initialization/decoder/transport, Web
initialization/transport, fixtures, schema generation and drift checking all use v16.
...
- Generated `protocol/app-server/v16.ts`, `v16.schema.json`, `fixtures.ts`, `fixtures.json`; removed v7 files under the single-current-version convention.
...
cannot be overridden by durable recovery. Runtime Client v40, App Server v16 and
...
cleanup-recovery caller; its behavior remains unchanged. App Server v16 and Runtime
...
No backend or protocol changes were made: App Server v16 and Runtime Client v40,
