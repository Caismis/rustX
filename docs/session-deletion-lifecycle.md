...
App Server v10 owns the public `session/deletePreview`, `session/delete`
...
`protocol/app-server/v10.ts` contract. There is no separate deletion SDK. Shared Rust/TypeScript
...
## Product and client behavior (App Server v10)
...
explicit recovery action. App Server v10 and native Runtime Client v40 are unchanged.
