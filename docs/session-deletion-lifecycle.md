...
App Server v12 owns the public `session/deletePreview`, `session/delete`
...
`protocol/app-server/v12.ts` contract. There is no separate deletion SDK. Shared Rust/TypeScript
...
## Product and client behavior (App Server v12)
...
explicit recovery action. App Server v12 and native Runtime Client v40 are unchanged.
