...
App Server v11 owns the public `session/deletePreview`, `session/delete`
...
`protocol/app-server/v11.ts` contract. There is no separate deletion SDK. Shared Rust/TypeScript
...
## Product and client behavior (App Server v11)
...
explicit recovery action. App Server v11 and native Runtime Client v40 are unchanged.
