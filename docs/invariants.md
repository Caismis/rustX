...
- **Single-generation liveness, budget, and integrity are three contracts, not
  ...
  retried and consumes the one shared corrective generation.

- **The degeneration detector is a strong-evidence guard, never a quality
  judge.** It is deterministic, uses bounded memory (a fixed rolling window per
  channel), does bounded work per generated byte, invokes no model, consults no
...
- **A generation budget is a byte safeguard, never a token count.** The
  ...
  provider enforces it. The runtime safeguard is derived from that same
  resolved budget and bounds normalized generated **bytes** — deliberately
  generously, because it exists to stop a runaway backend rather than to shape
  an answer. It is never described as, or substituted for, model-token
  accounting. The reasoning bound and the total-output bound remain
  semantically distinct and are reported as distinct typed facts.
...
- **Generation-safety facts are typed and structurally bounded.** Degeneration,
  budget exhaustion, and deadline expiry normalize into
  `ModelErrorKind::{GenerationDegenerated, GenerationBudgetExceeded, Timeout}`
  with a typed `ModelError.generation` payload whose fields are only
  enumerations and integers, so provider output can never author unbounded
  durable diagnostic data through them and no consumer above the model layer
  infers status from a message substring or a finish-reason string. They reuse
  `ModelRequestFailed`; no new top-level runtime event exists for them.
