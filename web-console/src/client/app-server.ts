...
export class AppServerClient {
  ...
  async connect(endpoint: string, token: string, transition: 'reconnect' | 'replace-authority' = 'reconnect', committed?: () => void) {
    ...
    committed?.();
    try {
      const socket = this.socketFactory(url.href, ['rustx.app-server.v18', `rustx-token.${token}`]);
      this.socket = socket;
      ...
      const hello = await this.request({ method: 'initialize', params: {
        protocol_version: 18, client: { name: 'rustx-web-console', version: '0.1.0' },
        presentation: { images: true, questionnaires: true, reviews: true },
      } }, 'initialized');
      if (!this.current(generation)) return;
      if (hello.protocol_version !== 18 || !hello.capabilities.multi_session || !hello.capabilities.headless_interactions || !hello.capabilities.single_writable_controller) {
        throw new Error('Incompatible App Server protocol or capabilities. Protocol v18 with native multi-Session, headless interactions, and single-controller admission is required.');
      }
  ...
  async setAgentModel(id: string, config: import('../../../protocol/app-server/v18').SessionModelConfig) {
