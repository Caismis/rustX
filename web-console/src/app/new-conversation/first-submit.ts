...
export const firstSubmitMachine = setup({
  ...
  on: { RETIRE: '.retired' },
  states: {
    drafting: { on: { SUBMIT: { guard: ({ context, event }) => context.port.current() && !!event.draft.workspaceId && (!!event.draft.text.trim() || event.draft.files.length > 0), target: 'creating_session', actions: assign({ draft: ({ event }) => event.draft, error: () => undefined }) } } },
