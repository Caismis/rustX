...
export function RuntimeFacts({ snapshot }: { snapshot: RuntimeClientSnapshot }) {
  ...
  return <section className="runtime-facts" aria-label={tx('agent:activity.current-activity')}>
    ...
    {background.map(tool => <ToolCard key={tool.execution_id} tool={{ id: tool.execution_id, identity: 'execution', title: tool.tool_name, variant: 'generic', summary: tx('agent:copy.background-task'),
