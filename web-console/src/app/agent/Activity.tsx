import type { RuntimeClientSnapshot } from '../../../../protocol/app-server/v6';
...
export function RuntimeFacts({ snapshot }: { snapshot: RuntimeClientSnapshot }) {
  ...
  if (!children.length && !workflows.length && !background.length && !snapshot.statuses?.length) return null;
  return <section className="runtime-facts" aria-label="Current activity">
    <small>Current activity</small>
    {background.map(tool => <ToolCard key={tool.execution_id} tool={{ id: tool.execution_id, identity: 'execution', title: tool.tool_name, variant: 'generic', summary: tool.execution_id,
