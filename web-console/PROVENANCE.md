...
| `ui-primitives/tests/{markdown-incremental,streaming-code-block}.client.spec.tsx` | `test/{markdown-incremental,streaming-code-block}.test.tsx` | Adapted deterministic upstream contracts to local imports/labels and selected grammars. Markdown-source highlighting tests excluded with that grammar; CRLF, token equality and DOM retention preserved. |
