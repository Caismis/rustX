...
const native = vi.hoisted(() => {
  ...
  class FakeTransport implements AppServerTransport {
    ...
    send(message: unknown) {
      ...
      if (request.method === 'initialize') queueMicrotask(() => this.deliver({ jsonrpc: '2.0', id: request.id, result: { type: 'initialized', protocol_version: 21, capabilities: { multi_session: true, single_writable_controller: true, headless_interactions: true, experimental_methods: [] } } }));
