...
it('closing A immediately emits exactly its detach, preserving B, runtime work and pending facts', async () => {
  ...
  expect(server.requests.slice(baseline).map(({ request }) => ({ method: request.method, params: request.params }))).toEqual([{ method: 'session/detach', params: { target } }]);
...
it('releasing an open tab also removes its existing reload hint without closing its presentation', async () => {
  ...
  const page = render(<App client={fresh.client} />); await act(() => fresh.connect());
