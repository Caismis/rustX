/* Copyright (c) 2026 DeepSeek. MIT; adapted tests, see PROVENANCE.md. */
...
import { CodeBlock } from '../src/presentation/markdown/CodeBlock.tsx'
import { StreamingHighlightSession } from '../src/presentation/markdown/highlight.ts'
const markdownLabels = { code: { copyLabel: 'Copy', copiedLabel: 'Copied' } }
...
describe('CodeBlock streaming arm', () => {
  ...
  it('a CRLF fence matches the settled swap', () => {
    const crlf = 'echo a \\\r\nb\r\nc\n'
  ...
  it('reuses an unchanged frame when an unrelated lazy grammar finishes loading', async () => {
    ...
    expect(loader.update('fn main() {}', 'rust')).toBeUndefined()
    await vi.waitFor(() => { expect(loader.update('fn main() {}', 'rust')).toBeDefined() }, { timeout: 5_000 })
