...
import type { AttachmentTarget, EffectiveConfiguration, MethodResult, Request1, SourceSettings } from '../../protocol/app-server/v18';
...
export function cfg3Client(handler?: (operation: Request1, source: SourceSettings, effective: EffectiveConfiguration) => Promise<MethodResult | void>) { const source = cfg3Source(), effective = cfg3Effective(), client = new AppServerClient(); const state: ClientView = { connection: 'connected', generation: 1, sessions: [], uncertain: [], interactionOperations: {}, views: { [cfg3Session]: { id: cfg3Session, attachmentIntent: 'wanted', attachment: 'attached', target: cfg3Target } } }; vi.spyOn(client, 'getSnapshot').mockReturnValue(state); const request = vi.spyOn(client, 'request').mockImplementation(async (operation) => { const overridden = await handler?.(operation, source, effective); if (overridden)
    ...
    }>).params.target; return { type: 'source_settings', projection: structuredClone(source) } as never; }); return { client, request, source, effective, state, host: undefined as import('../src/workspaces/host').ProductHostWorkspaces | undefined }; }
export function cfg3Host(subject: ReturnType<typeof cfg3Client>): import('../src/workspaces/host').ProductHostWorkspaces { return { listWorkspaces: async () => ({ endpoint: '', workspaces: ['A', 'B'].map(id => ({ id, displayName: id, location: id, displayPath: '/workspace/' + id })), picker: { kind: 'unavailable', reason: 'fixture' } }), configureWorkspace: async (id, _endpoint, operation) => { const target = { kind: 'workspace' as const, directory: '/workspace/' + id }; if (operation.kind === 'write')
        return (await subject.client.request({ method: 'configuration/sourceWrite', params: { target, expected_revision: operation.expected_revision, mutation: operation.mutation } }, 'source_settings')).projection; if (operation.kind === 'reconcile')
