...
it.each(['target', 'connection'] as const)('C10 late response after %s replacement cannot overwrite new authority', async invalidation => {
 let release!: (result: import('../../protocol/app-server/v23').MethodResult) => void;
 const pending = new Promise<import('../../protocol/app-server/v23').MethodResult>(resolve => { release = resolve; });
 let reads = 0;
...
it('an older authoritative read cannot replace a newer read', async () => {
  let release: (value: import('../../protocol/app-server/v23').MethodResult) => void = () => {};
  let count = 0;
...
it.each([
  ...
  const pending = new Promise<import('../../protocol/app-server/v23').MethodResult>((_, reject) => { rejectRead = reject; });
  let reads = 0;
...
it.each(['before acknowledgement', 'after acknowledgement', 'after the next edit'] as const)('T12/T16 source projection %s preserves subsequent drafts', async order => {
  ...
  let acknowledge!: (outcome: { acknowledgement: import('../../protocol/app-server/v23').SourceSettings }) => void;
  const held = new Promise<{ acknowledgement: import('../../protocol/app-server/v23').SourceSettings }>(resolve => { acknowledge = resolve; });
...
it('retires a confirmed Provider save, including its literal credential, after the editor unmounts', async () => {
  let release!: (result: import('../../protocol/app-server/v23').MethodResult) => void;
  const heldWrite = new Promise<import('../../protocol/app-server/v23').MethodResult>(resolve => { release = resolve; });
