...
it('an older authoritative read cannot replace a newer read', async () => {
  let release: (value: import('../../protocol/app-server/v11').MethodResult) => void = () => {};
  let count = 0;
...
it.each([
  ...
  const pending = new Promise<import('../../protocol/app-server/v11').MethodResult>((_, reject) => { rejectRead = reject; });
  let reads = 0;
