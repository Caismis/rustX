...
it('an older authoritative read cannot replace a newer read', async () => {
  let release: (value: import('../../protocol/app-server/v12').MethodResult) => void = () => {};
  let count = 0;
...
it.each([
  ...
  const pending = new Promise<import('../../protocol/app-server/v12').MethodResult>((_, reject) => { rejectRead = reject; });
  let reads = 0;
