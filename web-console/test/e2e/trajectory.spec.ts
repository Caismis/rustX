...
// When the loaded window carries no recorded timing, the duration projection
// has no model and the overview falls to its empty state. The earlier-history
// affordance is still the same control driven by the same paging authority,
// and it is absolutely positioned — so the overview must be its containing
// block, or it escapes and lands over unrelated chrome. jsdom cannot see that;
// a real layout can.
test('the empty timing overview keeps its earlier-history boundary inside itself', async ({ page }) => {
  const errors: string[] = [];
  page.on('pageerror', error => errors.push(error.message));
  await page.goto('http://127.0.0.1:5174/test/fixtures/trajectory.html?untimed');
  const ledger = page.getByRole('table', { name: 'Trace ledger' });
  const overview = page.getByLabel('Timing overview');
  await expect(ledger).toHaveAttribute('aria-rowcount', '6');

  // Native recorded nothing to place on a duration axis for this window.
  await page.getByRole('button', { name: 'Duration' }).click();
  await expect(overview).toBeVisible();
  await expect(overview.getByText('No recorded timing in the loaded window')).toBeVisible();

  const boundary = overview.getByLabel('Load earlier records into the overview');
  await expect(boundary).toBeVisible();
  const overviewBox = (await overview.boundingBox())!;
  const boundaryBox = (await boundary.boundingBox())!;
  expect(boundaryBox.x).toBeGreaterThanOrEqual(overviewBox.x - 1);
  expect(boundaryBox.y).toBeGreaterThanOrEqual(overviewBox.y - 1);
  expect(boundaryBox.x + boundaryBox.width).toBeLessThanOrEqual(
    overviewBox.x + overviewBox.width + 1,
  );
  expect(boundaryBox.y + boundaryBox.height).toBeLessThanOrEqual(
    overviewBox.y + overviewBox.height + 1,
  );
  // It spans the overview's own height, which is what the absolute rules ask
  // for — proof it resolved against the overview and not an outer ancestor.
  expect(boundaryBox.height).toBeGreaterThan(0);
  expect(Math.abs(boundaryBox.height - overviewBox.height)).toBeLessThanOrEqual(1);
  // Nothing overflows the document because of the absolute placement.
  expect(await page.evaluate(() => {
    const root = document.documentElement;
    return [root.scrollWidth - root.clientWidth, root.scrollHeight - root.clientHeight];
  })).toEqual([0, 0]);

  // The same single load-earlier operation, reached by keyboard.
  await boundary.focus();
  await expect(boundary).toBeFocused();
  await page.keyboard.press('Enter');
  await expect(ledger).toHaveAttribute('aria-rowcount', '10');
  // Still no timing, so the empty state stands; the fixture's older page was
  // the last one, so the affordance retires exactly as it does with a model.
  await expect(overview.getByText('No recorded timing in the loaded window')).toBeVisible();
  await expect(boundary).toHaveCount(0);
  expect(errors).toEqual([]);
});
