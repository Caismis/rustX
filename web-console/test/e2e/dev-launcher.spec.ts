...
`);
  ...
  try {
    ...
    await new Promise<void>((resolve, reject) => {
      const socket = new WebSocket(ready.endpoint, ['rustx.app-server.v13', `rustx-token.${launchToken}`]);
    ...
    await new Promise<void>((resolve, reject) => {
      const socket = new WebSocket(ready.endpoint, ['rustx.app-server.v13', `rustx-token.${proof}`]);
