...
import type { TraceDetail } from '../../protocol/app-server/v13';
import type { AttachmentTarget, MethodResult, Notification, Request, Response, RoutedInteraction, RuntimeClientSnapshot, SessionSummary, ServerCapabilities } from '../../protocol/app-server/v13';
...
export class Server {
  ...
  socketFactory = (_url: string, protocols: string[]) => {
    if (protocols[0] !== 'rustx.app-server.v13' || protocols[1] !== `rustx-token.${TOKEN}`) throw new Error('Wrong browser admission protocol');
