...
console, then **Connect**. The client sends `initialize` protocol v4, checks native
...
The browser sends subprotocols `rustx.app-server.v4` and `rustx-token.<token>` in its
...
- `src/client/`: one WebSocket, generated `protocol/app-server/v4.ts` unions,
