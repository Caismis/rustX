...
const schema = JSON.parse(await readFile(new URL('v17.schema.json', import.meta.url), 'utf8'));
// Schemars emits draft-2020-12 $ref siblings for internally tagged newtypes.
// v17's ref resolver merges (and overwrites) their `properties`, losing the
...
const types = await compile(normalizeRefSiblings(schema), 'ProtocolMessage', {
  ...
  // Schemars closes flattened envelopes with unevaluatedProperties. v17 does
...
await writeFile(new URL('v17.ts', import.meta.url), types);
