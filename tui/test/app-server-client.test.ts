...
describe("initialization", () => {
  it("negotiates the protocol version once and records server capabilities", async () => {
    ...
    assert.equal(APP_SERVER_PROTOCOL_VERSION, 10);
    assert.equal(params.protocol_version, 10);
  ...
  it("refuses a negotiated version other than the one it asked for", async () => {
    ...
    await assert.rejects(pending, /negotiated protocol 6, this client speaks 10/);
  });
});
